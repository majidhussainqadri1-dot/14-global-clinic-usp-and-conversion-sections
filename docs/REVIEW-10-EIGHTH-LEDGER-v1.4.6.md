...
| 10 | Exact-head closure exposed stale retained QA/evidence contracts tied to older prose/version identities, missing File 25/American-English traceability, and—critically—GitHub `pull_request` workflows were using the default synthetic merge ref while evidence language claimed the exact PR-head SHA. | Made retained regressions release-cycle forward-compatible without weakening invariants; restored exact-current-main/fresh-post-merge/live-state/File25/localization trace wording; changed quality, package, fresh-review and baseline PR jobs to checkout `${{ github.event.pull_request.head.sha || github.sha }}` and verify `git rev-parse HEAD` exactly; keyed the artifact name to the exact PR-head SHA; added regression assertions for these workflow invariants; reran all gates after the final documentation/ledger update. |
