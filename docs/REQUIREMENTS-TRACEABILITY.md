# File 14 — Fresh Central + File Plan + Future CTI Requirements Traceability — v1.4.2
...
|---|---|---|
| CEN-GOV-001 | Runtime records `GCU_PLAN_VERSION`, `GCU_FUTURE_PLAN_VERSION` and `GCU_CENTRAL_PLAN_BASELINE`; status model keeps coding separate from staging/live | `central-plan-tests.php`, future tests |
| CEN-OWN-001 | All doctor/onboarding/booking truth resolved through File 07/08/09 contracts; Future CTI stores only File14-owned governance records/reports | contract + central-plan + future tests |
...
| CEN-NAV-001 | File 20 remains sole global shell/navigation owner; Future CTI injects only semantic module sections | contract + central-plan + fresh-review tests |
...
| CEN-PRIV-001 | Consent/GPC, bounded attribution, small-cohort suppression, aggregate FAQ signals, PII-minimized copy reports | contract + central-plan + future tests |
...
| F14-FR-012 Experiment governance | Hypothesis/variants/audience/guardrails/sample/privacy and Founder-gated lifecycle | policy/contract + future tests |
...
| F14-FUT-04 | Claim Freshness Sentinel | Daily transactional stale/expired claim transition to `review_required`, public hide, history and audit; privileged revalidation path | future/fresh-review gates |
...
| F14-FUT-06 | Jurisdiction-Aware Truthful Copy Engine | Versioned `jurisdiction_copy` records; approved disclosure only; no eligibility or clinical decision | future records API + ownership review |
...
| F14-FUT-17 | Experiment Early-Stop & Rollback Guard | Automatically stops running experiments on high anomaly, policy drift, destination failure or complaint threshold; audited | hourly guard + fresh-review gate |
...
| F14-FUT-21 | AI Ethical Copy Assistant | Optional provider adapter receives approved claims; protected-concept/dark-pattern guard; deterministic fallback; no auto-publish | future tests + API |
| F14-FUT-22 | Public Clinic Trust & Change Log | Public material File14 policy/change records with review lifecycle | seeded public change log + API/public rendering |
...
|---|---|---|
| F14-NFR-001 Authorization | Native capabilities, object/purpose/state/version checks and fail-closed REST; Future write/admin endpoints reuse native capabilities | real-role/IDOR staging matrix |
| F14-NFR-002 Privacy lifecycle | Consent/GPC, minimization, export/erase, bounded attribution, non-destructive uninstall, cohort suppression, PII-minimized reports | WordPress privacy-tool staging acceptance |
| F14-NFR-003 Reliability | Idempotent event write, outbox/inbox, bounded retry/dead-letter; Future schema fail-safe, transactions and scheduled guards | queue/provider-failure staging test |
...
| F14-NFR-005 Accessibility | Script-free fallback nav, lang/dir, focus, 320px reflow, reduced motion/data, forced colors; Future UI has 44px/focus/reflow gates | screen reader, 320–1920px and 400% zoom acceptance |
| F14-NFR-006 Observability | Base health plus Future parity/anomaly/quality/consistency/report indicators | real logging/alert ownership acceptance |
| F14-NFR-007 Migration/rollback | Install lock, dbDelta, owner-scoped snapshots, safe repair; Future tables are additive and separately verified | fresh install/upgrade/concurrency/restore drill |
...
| F14-NFR-010 Localization | Complete File 14-owned en-US/ur-PK/ar-SA chrome plus protected terminology lock | human locale QA and translation sign-off |
...
- DoD-11 — two separate fresh review/fix rounds after final coding change: executable fresh review gates include Future CTI requirements; exact-head regression must be green after the final commit.
...
`Specified`, `Coded`, `Packaged`, `Automated-QA Green`, `Staging-Accepted`, `Live-Deployed`, and `Operational` are separate statuses. File 14 v1.4.2 may only claim a status for which current exact-head evidence exists. The Future CTI implementation does not alter that rule.
