...
root=Path('.')
contracts=root/'14-global-clinic-usp-integration/includes/class-gcu-contracts.php'
...
if s.count(old)!=1: raise SystemExit(f'contracts anchor count={s.count(old)}')
contracts.write_text(s.replace(old,new,1),encoding='utf-8')
...
$contracts=file_get_contents($root.'/14-global-clinic-usp-integration/includes/class-gcu-contracts.php');
...
r12r18_assert(false!==strpos($contracts,"gcu_inbound_contract_version_unsupported"),'Unknown inbound owner contract versions must fail closed.');
r12r18_assert(false!==strpos($contracts,"1!==\\$contract_version"),'Inbound v1 event handlers must enforce the exact supported contract version.');
