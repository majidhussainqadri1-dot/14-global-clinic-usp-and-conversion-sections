...
r12r12_assert(false!==strpos($policy,"$business = self::business_policy_contradiction_scan( $text );"),'Experiment variants must run the business-policy contradiction scanner.');
r12r12_assert(false!==strpos($policy,"array_merge( $scan['flags'], $business['flags'] )"),'Experiment preflight must merge dark-pattern and business-policy flags.');
