...
preg_match('/Version:\s*([0-9]+\.[0-9]+\.[0-9]+)/',$loader,$vm);preg_match("/define\(\s*'GCU_VERSION'\s*,\s*'([^']+)'/",$loader,$cm);$header_version=isset($vm[1])?$vm[1]:'';$const_version=isset($cm[1])?$cm[1]:'';
f4c($header_version===$const_version&&version_compare($header_version,'1.4.2','>='),'v1.4.2-or-later release identity missing or version drifted.');
