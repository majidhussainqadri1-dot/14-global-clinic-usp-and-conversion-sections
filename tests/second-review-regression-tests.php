...
sr_check( false !== strpos( $trace, 'Requirements Traceability — v1.4.3' ) && false !== strpos( $trace, 'File 14 v1.4.3 may only claim a status' ), 'Traceability current-version truth is stale.' );
sr_check( false !== strpos( $status, 'v1.4.3 Fifth-Review Repository Candidate' ) && false !== strpos( $status, 'Exact-current-head rule' ) && false === strpos( $status, 'Corrective Candidate — Merged' ), 'Status wording remains contradictory.' );
sr_check( false !== strpos( $release, 'v1.4.3 Fifth-Review Repository Candidate' ) && false !== strpos( $release, 'exact review/main SHA being accepted' ) && false !== strpos( $release, 'fresh post-merge' ), 'Release evidence does not enforce current exact-head/post-merge truth.' );
