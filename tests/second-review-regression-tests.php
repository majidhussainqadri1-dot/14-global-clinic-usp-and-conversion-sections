...
sr_check( '' !== $version && false !== strpos( $trace, 'Requirements Traceability — v' . $version ) && false !== strpos( $trace, 'File 14 v' . $version . ' may only claim a status' ) && false !== strpos( $trace_addendum, 'Requirements Traceability Addendum — v1.4.5' ), 'Current traceability or retained v1.4.5 delta trace is missing.' );
sr_check( '' !== $version && false !== strpos( $status, 'v' . $version ) && false !== strpos( $status, 'Repository Candidate' ) && false !== strpos( $status, 'Exact-current-head rule' ) && false !== strpos( $status, 'exact resulting `main` SHA' ) && false === strpos( $status, 'Corrective Candidate — Merged' ), 'Status wording remains contradictory or exact-head rule is missing.' );
sr_check( '' !== $version && false !== strpos( $release, 'v' . $version ) && false !== strpos( $release, 'Repository Candidate' ) && false !== strpos( $release, 'exact current PR-head SHA' ) && false !== strpos( $release, 'After merge' ), 'Release evidence does not enforce current exact-head/post-merge truth.' );
